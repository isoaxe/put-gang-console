module.exports.Database = class Database {
    ...
    set(_doc, _data){
        return new Promise((resolve, reject) => {
            if(!_doc){ return reject(new Error("No document name Provided")) }
            if(!_data){ return reject(new Error("No Data Provided")) }
            if(typeof _doc != "string"){ return reject(new Error("Invalid Document name")) }
            if(typeof _data != "object"){ return reject(new Error("Invalid Data")) }
            this.db.doc(_doc).set(_data)
                .then(resolve)
                .catch(reject)
        })
    }
