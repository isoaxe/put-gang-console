...
const InvoicesList = (props) => {
  ...
  function formatStatement(name, email, sale, product) {
    return `${name ? name : email} made a $${sale} payment for ${capitalize(
      product
    )} the Discussion.`;
  }
  ...
  const formatInvoicesData = useCallback(() => {
    ...
    combinedInvoices.forEach(
      (item) =>
        (item["statement"] = formatStatement(
          item.name,
          item.email,
          item.sale,
          item.product
  ...
  useEffect(() => {
    if (
      users.length &&
      invoices.length &&
      ["admin", "level-1", "level-2"].includes(role)
    ) {
      formatInvoicesData();
    }
  ...
  return (
    <Container className="list">
      <Box mb={2}>
        <ListSearchbar handleInputChange={handleInputChange}></ListSearchbar>
      </Box>
      <Hidden xsDown>
